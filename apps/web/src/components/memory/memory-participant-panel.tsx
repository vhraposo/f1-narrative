...
type MemoryParticipantPanelProps = {
  memoryId: string;
};

export function MemoryParticipantPanel({
  memoryId,
}: MemoryParticipantPanelProps) {
  const memoryQuery = useMemory(memoryId);
  const charactersQuery = useCharacters();
  const addMutation = useAddMemoryParticipant(memoryId);
  const removeMutation = useRemoveMemoryParticipant(memoryId);
  ...
  const participants = memoryQuery.data?.participants ?? [];
      ...
      onError: (err) => {
        ...
        setFormError(
          err instanceof ApiError && err.status === 409
            ? "Este personagem já participa da memória."
            : message,
        );
      },
    });
        ...
        {charactersQuery.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            {characters.length === 0
              ...
              : "Todos os seus personagens já participam desta memória."}
          </p>
        ) : (
          <form
